...
## 6-1. Canary 체크 항목 (코드 리뷰 발견, 배포 후 확인)
- **빗썸 매수 수수료가 코인으로 차감되는 경우**: `coin_neutral` 매도 수량 = 매수 `executed_volume` 전량인데, 거래소가 매수 수수료를 코인으로 떼면 실제 보유 코인 < 매도 수량 → 매도 잔고 부족 실패 가능(자금 손실은 없고 봇 정지성). 업비트는 KRW로 수수료를 떼어 안전. **빗썸에서 coin_neutral 봇을 처음 가동할 때 첫 매도 사이클이 잔고 부족 없이 체결되는지 반드시 확인**할 것. (현재 빗썸 수수료 무료(0%)면 무관)
- **profitMode 토글 반영 지연**: `botInfoCache` TTL(5분) 때문에 opposite-order 경로는 최대 5분 지연 반영(주기 매도는 즉시). 신규 봇 생성에는 영향 없음.
