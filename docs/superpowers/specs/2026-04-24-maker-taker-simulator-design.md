...
## 10. 열린 결정 사항 (2026-04-24 세션 8 시작 시 확정)

- [x] **체결 판단 방식**: **A** (`best bid ≤ 내 지정가`면 체결 간주)
  - 근거: MVP로 이론 상한부터 확인. 수익성 존재 여부가 우선. 48h 관찰 후 필요 시 D 업그레이드.
- [x] **초기 봇 개수**: **3개** (offset -2 / -4 / -6)
  - 근거: 한 세트 관찰로 최적 offset 도출. DB 볼륨 미미 (3봇 × 48h × 1trade/h = 최대 144 rows).
- [x] **minTakerBidKrw 정책**: **하드코딩 1485** (MVP)
  - 근거: 동적 계산 대비 구현/테스트 간단. 봇 테이블에 필드 존재하므로 실거래 전환 시 값만 수정.
- [x] **Upbit trades API 사용**: **미사용**
  - 근거: 방식 A 채택으로 불필요. rate limit 10req/sec 이슈 회피. 방식 D 업그레이드 시에만 WS trade 채널 구독 도입.
