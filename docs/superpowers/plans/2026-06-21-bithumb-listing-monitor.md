...
## Task 1: Prisma 스키마에 source enum 추가 + 마이그레이션

**Files:**
- Modify: `prisma/schema.prisma` (line 571~656 영역)
- Create: `prisma/migrations/<timestamp>_add_listing_source_column/migration.sql` (Prisma CLI 자동 생성)

**기존 모델 (변경 전 — 실측)**:
- `UpbitListingAnnouncement` (table `upbit_listing_announcements`) — `noticeId Int @unique`
- `ListingAutoTradeConfig` (table `listing_auto_trade_config`) — 싱글톤 (id=1), userId/killSwitch/minTakerBalance 컬럼 없음
- `ListingAutoOrder` (table `listing_auto_orders`)

**변경 원칙**:
- 모델/테이블명 유지 (production 데이터 보존)
- `ListingAutoTradeConfig`: 싱글톤 → `source @unique` 멀티-row
- `userId` 추가 안 함 (코드가 `ADMIN_USER_ID=2` 하드코딩)
- `killSwitch`, `minTakerBalance`는 NEW 컬럼

- [ ] **Step 0: 사전 검증 (advisor 권고)**

Run: `node -e "const p=require('@prisma/client').PrismaClient; new p().\$queryRaw\`SELECT COUNT(*) FROM upbit_listing_announcements WHERE noticeId IS NULL\`.then(r=>console.log(r))"`

또는 prisma studio 등으로 dev DB에서 확인.
Expected: `[{ count: 0n }]` — NULL noticeId 0건. 0건이 아니면 unique 제약 변경 시 에러 발생하므로 마이그레이션 전에 NULL 처리 필요. BLOCKED 보고.

- [ ] **Step 1: 스키마 수정**

`prisma/schema.prisma`에서 line 571~656 영역을 다음 내용으로 교체.

먼저 enum 추가 (다른 enum들 옆, 예: line 568 근처 또는 상단):
```prisma
enum ListingSource {
  UPBIT
  BITHUMB
...
`UpbitListingAnnouncement` 모델 (line 571~587 영역) 수정:
```prisma
...
(주요 변경: `noticeId Int @unique` → `noticeId Int` + `@@unique([source, noticeId])` 추가)

`ListingAutoTradeConfig` 모델 (line 590~607 영역) 수정:
```prisma
model ListingAutoTradeConfig {
  id              Int           @id @default(autoincrement())
  source          ListingSource @unique @default(UPBIT)
...
(추가된 컬럼: `source ListingSource @unique @default(UPBIT)`, `killSwitch Boolean @default(false)`, `minTakerBalance Float?`. 기존 default 값은 그대로 유지 — 업비트 row 백필 시 기존 값 보존)

`ListingAutoOrder` 모델 (line 610~639 영역) 수정 — 기존 필드 그대로 + 아래 추가:
```prisma
model ListingAutoOrder {
  // ... 기존 필드 그대로 ...
  source         ListingSource @default(UPBIT)
  // ... 기존 announcement relation 그대로 ...

  @@unique([announcementId, exchange])
  @@index([status])
  @@index([sellStatus])
  @@index([createdAt])
  @@index([source, createdAt])
  @@map("listing_auto_orders")
}
```

(추가: `source ListingSource @default(UPBIT)` + `@@index([source, createdAt])`)
...
생성된 `migration.sql` 파일을 Read tool로 열어서:
1. CLAUDE.md 글로벌 규칙(Prisma migrate CLI garbage 패턴) 따라 박스 문자(│ ─ ━ 등) 혼입 여부 확인
2. SQL 내용이 아래와 비슷한지 확인 (Prisma가 자동 생성한 인덱스 이름은 환경마다 다를 수 있음 — 실제 생성된 이름 그대로 사용):

```sql
-- 1) source 컬럼 추가 (DEFAULT 'UPBIT'로 기존 row 자동 백필)
...
-- 2) Unique 제약 교체 (upbit_listing_announcements.noticeId @unique → composite)
...
-- 3) listing_auto_trade_config: source @unique 추가
ALTER TABLE `listing_auto_trade_config`
  ADD UNIQUE INDEX `listing_auto_trade_config_source_key` (`source`);

-- 4) 인덱스 추가
ALTER TABLE `upbit_listing_announcements` ADD INDEX `upbit_listing_announcements_source_announcedAt_idx` (`source`, `announcedAt`);
ALTER TABLE `listing_auto_orders` ADD INDEX `listing_auto_orders_source_createdAt_idx` (`source`, `createdAt`);
```

박스 문자 등 garbage가 있으면 Edit tool로 제거.
...
- [ ] **Step 5: Prisma 클라이언트 재생성 (자동 호출되지만 명시 확인)**
...
Run: `node -e "const p=require('@prisma/client').PrismaClient; new p().listingAutoTradeConfig.findFirst().then(r=>console.log(r))"`
Expected: 기존 row가 `source: 'UPBIT'`, `killSwitch: false`, `minTakerBalance: null`로 백필되어 있음
...
git commit -m "feat: 신규상장 테이블에 source enum + killSwitch/minTakerBalance 컬럼 추가"
```
...
async pollBithumbTelegram(): Promise<void> {
  try {
    ...
    for (const msg of messages) {
      ...
      // Insert (table은 "upbit_listing_announcements" 그대로 사용 — source='BITHUMB'로 분리)
...
`getConfig` 시그니처 변경 (source @unique 사용, userId 없음):
```typescript
async getConfig(source: 'UPBIT' | 'BITHUMB' = 'UPBIT'): Promise<AutoTradeConfig> {
  ...
  if (!row) {
    ...
    if (source === 'BITHUMB') {
      return {
        source: 'BITHUMB',
        enabled: false, killSwitch: false, amountKrw: 10000,
    ...
    return {
      source: 'UPBIT',
      enabled: false, killSwitch: false, amountKrw: 100000,
...
async trigger(announcement: { source: 'UPBIT' | 'BITHUMB'; ticker: string; ... }): Promise<void> {
  ...
  // 24h 중복 매수 방지
  const duplicate = await (prisma as any).listingAutoOrder.findFirst({
    where: {
  ...
  await (prisma as any).listingAutoOrder.create({
    data: {
...
async checkAndSell(): Promise<void> {
  // 기존 status 값 그대로 사용 (실제 코드에서 사용하는 status 값에 맞춰 조회)
  const openOrders = await (prisma as any).listingAutoOrder.findMany({
    where: { sellStatus: null, status: 'filled' },  // 매수 체결됐고 아직 매도 안 함
  });
...
`bithumb-listing-monitor.service.ts`에 `listAnnouncements`/`getAnnouncement` 메소드 추가 — 단순 `prisma.upbitListingAnnouncement.findMany({ where: { source: 'BITHUMB' }, ...})` 패턴.
