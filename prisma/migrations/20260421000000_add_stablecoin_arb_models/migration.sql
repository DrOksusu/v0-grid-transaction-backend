-- AlterTable: credentials enum에 stablecoin_arb 값 추가
ALTER TABLE `credentials` MODIFY `purpose` ENUM('default', 'infinite_buy', 'stablecoin_arb', 'vr') NOT NULL DEFAULT 'default';

-- CreateTable: 스테이블코인 아비트리지 봇 설정
...
-- CreateTable: 스테이블코인 아비트리지 개별 거래 기록
...
-- CreateTable: 스테이블코인 아비트리지 기회 로그
...
-- AddForeignKey: stablecoin_arb_bots → users
ALTER TABLE `stablecoin_arb_bots` ADD CONSTRAINT `stablecoin_arb_bots_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `users`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey: stablecoin_arb_trades → stablecoin_arb_bots
...
-- AddForeignKey: stablecoin_arb_opportunities → stablecoin_arb_bots
