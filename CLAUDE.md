...
### Task 도구로 서브에이전트 호출 시 필수 규칙
서브에이전트를 Task 도구로 호출할 때, 반드시 아래 지침을 프롬프트에 포함할 것:
- **백엔드 작업**: "먼저 `v0-grid-tranasction-backend/CLAUDE.md`를 읽고 그 안의 규칙을 따를 것"
- **프론트엔드 작업**: "먼저 `v0-grid-transaction-frontend/CLAUDE.md`를 읽고 그 안의 규칙을 따를 것"
- **양쪽 모두 수정**: 각각 별도 서브에이전트로 분리하여 병렬 호출할 것
